...
export class ProfileView extends React.Component {
  ...
  getUser(token) {
    let url = 'https://sharmilamovie.herokuapp.com/users/' +
  ...
  handleRemove(movie) {
    ...
    axios.post(`https://sharmilamovie.herokuapp.com/users/${user}/` +
      movie._id, {},
      { headers: { Authorization: `Bearer ${token}` } }
    )
      .then((response) => {
  ...
  handleDelete() {
    ...
    if (answer) {
      const token = localStorage.getItem("token");
      const user = localStorage.getItem("user");
        axios.delete( `https://sharmilamovie.herokuapp.com/users/${user}`,
          { headers: { Authorization: `Bearer ${token}` } }
          )
          .then(() => {
